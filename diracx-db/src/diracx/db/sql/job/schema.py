...
class Jobs(JobDBBase):
    ...
    submission_time = NullColumn("SubmissionTime", DateTime(timezone=True))
    reschedule_time = NullColumn("RescheduleTime", DateTime(timezone=True))
    last_update_time = NullColumn("LastUpdateTime", DateTime(timezone=True))
    start_exec_time = NullColumn("StartExecTime", DateTime(timezone=True))
    heart_beat_time = NullColumn("HeartBeatTime", DateTime(timezone=True))
    end_exec_time = NullColumn("EndExecTime", DateTime(timezone=True))
...
class HeartBeatLoggingInfo(JobDBBase):
    ...
    name = Column("Name", String(100), primary_key=True)
    value = Column("Value", Text)
    heart_beat_time = Column("HeartBeatTime", DateTime(timezone=True), primary_key=True)
...
class JobCommands(JobDBBase):
    ...
    reception_time = Column("ReceptionTime", DateTime(timezone=True), primary_key=True)
    execution_time = NullColumn("ExecutionTime", DateTime(timezone=True))
